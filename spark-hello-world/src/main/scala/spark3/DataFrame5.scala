...
object DataFrame5 extends App {
  ...
  /*
  +-----+------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  |name |course|array_sort(grade, lambdafunction((IF(((namedlambdavariable() IS NULL) AND (namedlambdavariable() IS NULL)), 0, (IF((namedlambdavariable() IS NULL), 1, (IF((namedlambdavariable() IS NULL), -1, (IF((namedlambdavariable() < namedlambdavariable()), -1, (IF((namedlambdavariable() > namedlambdavariable()), 1, 0)))))))))), namedlambdavariable(), namedlambdavariable()))|
  +-----+------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
  ...
  +-----+------+--------------------------------------------------------------------------------+
  |Amy  |OS    |[{45, 92}, {69, 96}, {79, 100}, {89, NULL}, {92, NULL}, {92, NULL}, {100, NULL}]|
  |Bob  |OS    |[{77, 98}, {77, 99}, {88, 99}, {88, NULL}, {99, NULL}, {99, NULL}]              |
  |Cathy|OS    |[{NULL, 95}]                                                                    |
  |David|OS    |NULL                                                                            |
  |David|AI    |NULL                                                                            |
  +-----+------+--------------------------------------------------------------------------------+
  ...
  // must set this in spark 3
  ss.conf.set("spark.sql.mapKeyDedupPolicy", "LAST_WIN")
  ...
  /*
  +-----+------+--------------------+
  |name |course|struct(name, course)|
  +-----+------+--------------------+
  |Amy  |OS    |{Amy, OS}           |
  |Bob  |OS    |{Bob, OS}           |
  |Cathy|OS    |{Cathy, OS}         |
  ...
  root
   |-- name: string (nullable = true)
   |-- course: string (nullable = true)
   |-- struct(name, course): struct (nullable = false)
   |    |-- name: string (nullable = true)
   |    |-- course: string (nullable = true)
   */
